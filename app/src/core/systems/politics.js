...
export function deserializePolitics(data, rng) {
  ...
  if (data.ruler && Array.isArray(data.ruler.traits)) {
    s.ruler = {
      ...
      // НИЖНЯЯ ГРАНИЦА 0, А НЕ 14. Здесь стояло 14, и пока правителей делал
      // только createRuler (он даёт взрослых), это ничего не значило. С
      // подключением рода на трон законно садится ребёнок: наследник принимает
      // власть в том возрасте, в каком застала его смерть отца, и панель «Род»
      // так и пишет — «Хорс Косматый, 9 лет».
      //
      // Порог ломал сейв. Род сохранял правителя 11 лет (1100 дней), politics
      // при загрузке поднимал его до 14, и второе сохранение уже не совпадало
      // с первым — круг рвался на ровном месте. Поймалось на test-worldsites.
      //
      // Ничего, кроме порчи данных, эта граница не давала: единственный, кто
      // читает возраст, — deathChance, а он до пятидесяти возвращает одно и то
      // же число. Верхняя граница 120 остаётся: она отсекает мусор.
      age: clamp(num(data.ruler.age, 30), 0, 120),
