...
export function empireRestore(sim, data) {
  ...
  // ТУМАН ПОСЛЕ ЗАГРУЗКИ НЕ ДООТКРЫВАЕМ, ЕСЛИ ОН ПРИШЁЛ ИЗ СЕЙВА. Здесь стоял
  // безусловный empireFog(sim), и он ломал круг сохранения.
  //
  // Разобранный случай. Сейв снимают в произвольный миг суток — жители к этому
  // времени уже отошли от тех мест, где их застал последний дневной проход
  // тумана. Загрузка восстанавливала записанный туман и тут же открывала карту
  // ЕЩЁ РАЗ, по нынешним местам жителей. Получалось на несколько клеток больше,
  // чем было в исходной партии, и второе сохранение с первым не сходилось.
  // В отчёте это выглядело сдвигом одной границы в упакованном тумане:
  // ...72,1,22,0,73... против ...72,1,23,0,72...
  //
  // Записанный туман — истина: игра его уже посчитала, повторять нечего.
  // Проход остаётся только для сейвов, где тумана нет вовсе (старые версии):
  // там его иначе неоткуда взять, и партия начнётся с чёрной картой.
  if (!data.sites || !sim.sites) empireFog(sim);
  return true;
}
