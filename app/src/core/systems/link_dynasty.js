...
export const PLOT_STEP_MAX = 1.6;        // потолок ПРИРОСТА заговора за сутки
...
// Предел спада за сутки: полный спад при выправленном положении плюс надбавка
// за любимого правителя. Больше этого calm не бывает — величина выведена из
// самих слагаемых, а не назначена отдельным числом.
export const PLOT_STEP_CALM = PLOT_CALM + KIND_PLOT_CALM;
...
export function plotDelta(st) {
  ...
  // Потолок ПРИРОСТА. Без него один чёрный день (пустой трон плюс два
  // претендента плюс обиженные сословия) поднимал бы заговор на 3.15 за сутки,
  // и от первой ступени до переворота проходило бы меньше месяца — игрок не
  // успевал бы даже прочитать предупреждение.
  //
  // ПОТОЛОК ОДНОСТОРОННИЙ, И ЭТО НАМЕРЕННО. Здесь стояло
  // clamp(..., -PLOT_STEP_MAX, PLOT_STEP_MAX), и оно тихо срезало спад до тех
  // же 1.6 — притом что весь смысл PLOT_CALM = 1.9 в том, чтобы выправленное
  // положение гасило смуту БЫСТРЕЕ, чем она копилась. С прежним потолком
  // обещание из шапки («со 100 до нуля за 53 дня») не выполнялось: выходило
  // 62.5 дня, дольше самого быстрого роста. Вниз шаг ограничен своей
  // естественной величиной — больше PLOT_STEP_CALM спад дать неоткуда.
  const d = clamp(grow - calm, -PLOT_STEP_CALM, PLOT_STEP_MAX);
