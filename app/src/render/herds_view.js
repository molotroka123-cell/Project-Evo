...
// Пикселей мирового буфера зон на клетку — см. _drawZones.
const ZONE_PPT = 4;
...
export class HerdsView {
  ...
  constructor(quality, animals) {
    ...
    this._zoneCv = null;       // мировой буфер зон выпаса
    this._zoneKey = '';        // подпись состояния стад, по которой он перепекается
    this._zoneBox = null;      // объединяющий прямоугольник зон, в клетках
  ...
  setQuality(q) {
    ...
    this.atlas.clear();
    this.sprites.clear();
    // Буфер зон печётся из this.sprites — вместе с ними он и протух.
    this._zoneCv = null; this._zoneKey = ''; this._zoneBox = null;
  }
  ...
  drawGround(sim, ctx, ox, oy, z, cw, ch) {
    ...
    // --- зона выпаса ------------------------------------------------------
    // Читается как «здесь выбита трава и натоптано»: тон берётся от травы
    // этого сезона, а не выдумывается.
    //
    // ПОЧЕМУ НЕ ПО ПЯТНУ НА СТАДО, КАК БЫЛО. Так и было написано сначала:
    // цикл по стадам, на каждое один drawImage. Замер это отменил. Стад в
    // зрелой партии пятнадцать, радиусы 6–9 клеток, и при зуме 1 каждое пятно
    // кладётся квадратом 384–576 px. Суммарно — 3 млн полупрозрачных пикселей
    // в кадр при экране в 1,44 млн: игра дважды закрашивала весь экран ради
    // украшения земли. По замеру (максимум из трёх прогонов tools/shot.mjs)
    // сцена map шла 46 кадров против 55 без зон вовсе, start — 54 против 61.
    //
    // Выключение сглаживания, первая попытка, дало 49 вместо 46: дело было не
    // в фильтре, а в самой площади. Площадь и убираем: все пятна печём ОДИН
    // раз в мировой буфер (4 px на клетку — для мягкого пятна с запасом) и
    // кладём в кадр одним блитом по объединяющему прямоугольнику. Пятнадцать
    // перекрывающихся кругов складываются в объединение, а не в сумму, всё,
    // что вне экрана, отсекается прямоугольником, и в кадре остаётся ровно
    // один drawImage вместо пятнадцати.
    //
    // Перепечь буфер стоит около 47 тыс. пикселей — вдвое дешевле одного
    // прежнего пятна, — поэтому она не нормируется: буфер просто пересобирают,
    // когда стада сдвинулись хоть на четверть клетки.
    this._drawZones(sim, ctx, ox, oy, z, cw, ch, season, herds);
  ...
  // =========================================================================
  // -------------------------------------------------------------------------
  // Зоны выпаса одним блитом.
  //
  // Буфер живёт в МИРОВЫХ координатах, а не в экранных: от панорамы он не
  // зависит вовсе, и при движении камеры не перепекается ни разу. Пересборка
  // нужна только когда сдвинулось само стадо, сменился сезон или размер мира.
  //
  // ZONE_PPT — пикселей буфера на клетку. Четыре: пятно радиусом семь клеток
  // ложится в буфер кругом в 56 px, а внутри у него мягкий градиент без единой
  // резкой грани — растянуть такой круг обратно на 448 px можно без потерь,
  // которые различил бы глаз. Мир 96×96 даёт буфер 384×384, это 590 КБ.
  // -------------------------------------------------------------------------
  _drawZones(sim, ctx, ox, oy, z, cw, ch, season, herds) {
    const W = sim.world.w | 0, H = sim.world.h | 0;
    if (!(W > 0 && H > 0) || !herds.length) { this._zoneBox = null; return; }

    // Подпись состояния. Положение округляется до четверти клетки — это ровно
    // один пиксель буфера, то есть мельче округлять нечего: более точное
    // значение в буфере всё равно не отобразится, а перепечки шли бы каждый
    // кадр. Испуг и обречённость меняют и размер, и прозрачность, поэтому
    // входят тоже.
    let key = `${season}|${W}x${H}|${this.q.id}`;
    for (const h of herds) {
      key += `|${h.id},${Math.round(h.x * 4)},${Math.round(h.y * 4)},`
           + `${Math.round(h.r * 4)},${Math.round(h.fear * 20)},${h.doomed ? 1 : 0}`;
    }

    if (key !== this._zoneKey || !this._zoneCv) {
      const bw = Math.max(8, W * ZONE_PPT), bh = Math.max(8, H * ZONE_PPT);
      if (!this._zoneCv || this._zoneCv.width !== bw || this._zoneCv.height !== bh) {
        const cv = document.createElement('canvas');
        if (!cv) { this._zoneBox = null; return; }
        cv.width = bw; cv.height = bh;
        this._zoneCv = cv;
      }
      const c = this._zoneCv.getContext('2d');
      if (!c) { this._zoneBox = null; return; }
      ...
      for (const h of herds) {
        ...
        c.globalAlpha = 0.30 * (h.doomed ? 0.55 : 1) * (1 - 0.25 * h.fear);
        const cv = this.zoneSprite(season, h.fear >= FEAR_ALERT, h.doomed);
        c.drawImage(cv, (h.x - rr) * ZONE_PPT, (h.y - rr) * ZONE_PPT,
          rr * 2 * ZONE_PPT, rr * 2 * ZONE_PPT);
        if (h.x - rr < x0) x0 = h.x - rr;
        if (h.y - rr < y0) y0 = h.y - rr;
        if (h.x + rr > x1) x1 = h.x + rr;
        if (h.y + rr > y1) y1 = h.y + rr;
      }
      c.globalAlpha = 1;
      this._zoneKey = key;
      this._zoneBox = x1 > x0
    ...
    // Экранный прямоугольник объединения, обрезанный по кадру. Именно обрезка
    // и даёт выигрыш: стада стоят на своей части карты, и обычно в кадр входит
    // хорошо если половина.
    const sx0 = Math.max(0, ox + box.x0 * z), sy0 = Math.max(0, oy + box.y0 * z);
    const sx1 = Math.min(cw, ox + box.x1 * z), sy1 = Math.min(ch, oy + box.y1 * z);
    if (!(sx1 > sx0 && sy1 > sy0)) return;

    // Обратный перевод экранного прямоугольника в пиксели буфера.
    const k = ZONE_PPT / z;
    const bx = (sx0 - ox) * k, by = (sy0 - oy) * k;
    const bwv = (sx1 - sx0) * k, bhv = (sy1 - sy0) * k;

    // Сглаживание выключено по тому же доводу, что у кисти ореола в postfx.js:
    // растягиваем мягкий градиент, резких краёв в нём нет, а фильтр стоит
    // вшестеро дороже.
    const smooth = ctx.imageSmoothingEnabled;
    if ('imageSmoothingEnabled' in ctx) ctx.imageSmoothingEnabled = false;
    ctx.drawImage(this._zoneCv, bx, by, bwv, bhv, sx0, sy0, sx1 - sx0, sy1 - sy0);
    if ('imageSmoothingEnabled' in ctx) ctx.imageSmoothingEnabled = smooth;
  }
